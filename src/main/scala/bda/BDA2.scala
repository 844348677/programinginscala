...
object BDA2 {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("NewMethod").setMaster("spark://10.0.0.151:7077").setJars(List("D:\\workspace4scala\\rbda\\out\\artifacts\\rbda_jar\\rbda.jar")).set("spark.executor.memory", "8g").set("spark.executor.cores","8")
    val sc = new SparkContext(conf)

    val colmSize :Int =  1566
    val rowSize :Int = 139
    //生成bda2随机数列 2-rowSize,穷举,第一行为Y值 ！！后面有加1 rowSize+1
    val bda2 =List.range(2,rowSize+1) flatMap (i => List.range(2,i) map (j => i+" "+j ))
    val lines = sc.parallelize(bda2)
