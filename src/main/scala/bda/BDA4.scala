...
object BDA4 {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("NewMethod").setMaster("spark://10.0.0.151:7077").setJars(List("D:\\workspace4scala\\rbda\\out\\artifacts\\rbda_jar\\rbda.jar")).set("spark.executor.memory", "12g")
    val sc = new SparkContext(conf)

    val colmSize :Int =  1566
    val rowSize :Int = 139
    //生成bda4随机数列 2-rowSize,穷举,第一行为Y值 39	118	138
    //val bda4 = for(i <- List.range(2,rowSize+1); j <- List.range(2,i); k <- List.range(2,j);l <- List.range(2,k)) yield i+" "+j+" "+k+" "+l
    /*
        val pw = new PrintWriter(new File("bda4.txt"))
        val bda4 = for(i <- List.range(2,139+1); j <- List.range(2,i); k <- List.range(2,j);l <- List.range(2,k)){
          pw.write(i+" "+j+" "+k+" "+l)
          pw.write("\r\n")
        }
        pw.close()
    ...
    val lines = sc.textFile("hdfs://10.0.0.151:9000/input/bda4.txt")
    ...
    sorted.repartition(1).saveAsTextFile("hdfs://10.0.0.151:9000/output/20160411-01")
